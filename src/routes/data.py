...
async def process_endpoint(request : Request, project_id: str , process_request : ProcessRequest):
    ...
    print(f"🧐 Debugging Reset: do_reset = {do_reset} (Type: {type(do_reset)})")
    if do_reset == 1:
        print(f"🗑️ Starting delete for Project ID: {project.id} (Type: {type(project.id)})")
        
        deleted_count = await chunk_model.delete_chunks_by_project_id(project_id=project.id)
        
        print(f"✅ Deleted Count: {deleted_count}")
    else:
        print("⚠️ Reset skipped because condition didn't match!")
